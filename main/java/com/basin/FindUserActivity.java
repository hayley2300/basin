...
public class FindUserActivity extends AppCompatActivity {
    ...
    LinearLayout finduser_linear1, finduser_linear2;
    TextView finduser_findidpw, finduser_cancel1, finduser_cancel2, finduser_modify ;
    RadioGroup login_radiogroup;
    RadioButton r_btn1, r_btn2, r_btn3;
    int radio_mode=1;
    EditText finduser_email, finduser_findpw, finduser_findpwre, finduser_4digit, finduser_findid;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.login_finduser);
        mContext = this;
        finduser_linear1 = findViewById(R.id.finduser_linear1);
        finduser_linear2 = findViewById(R.id.finduser_linear2);
        finduser_findidpw = findViewById(R.id.finduser_findidpw);
        finduser_cancel1 = findViewById(R.id.finduser_cancel1);
        finduser_cancel2 = findViewById(R.id.finduser_cancel2);
        finduser_modify = findViewById(R.id.finduser_modify);
        ...
        finduser_email  = (EditText) findViewById(R.id.finduser_email);
        finduser_findpw = (EditText) findViewById(R.id.finduser_findpw);
        finduser_findpwre = (EditText) findViewById(R.id.finduser_findpwre);
        finduser_4digit = (EditText) findViewById(R.id.finduser_4digit);
        finduser_findid = (EditText) findViewById(R.id.finduser_findid);
        ...
        login_radiogroup.setOnCheckedChangeListener( new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, @IdRes int i) {
                if(i == R.id.rg_btn1){
                    radio_mode =1;
                    Toast.makeText(mContext, "라디오 그룹 버튼1 눌렸습니다.", Toast.LENGTH_SHORT).show();
                }
        ...
        finduser_findidpw.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(radio_mode!=1){

                    if(finduser_email.getText().toString().trim().equals("")){
                        Toast.makeText(mContext, "이메일을 입력해주세요".toString(),Toast.LENGTH_LONG).show();
                    }else if(!email_validator.isValidEmail(finduser_email.getText().toString().trim())){
                        // 이메일 형식 확인해야함
                        Toast.makeText(mContext, "이메일 형식이 맞지 않습니다.".toString(),Toast.LENGTH_LONG).show();
                    }else{
                        emailTask = new BackgroundTask();
                        emailTask.execute();
                        finduser_linear2.setVisibility(View.VISIBLE);
                        finduser_linear1.setVisibility(View.INVISIBLE);
                    }
                }else{
                            ...
                            if(finduser_email.getText().toString().trim().equals("")){
                                ...
                                ((FindUserActivity) mContext).finish();
                            }
                }
            }
        });
        finduser_modify.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(finduser_findid.getText().toString().trim().equals("")){
                    Toast.makeText(mContext, "아이디를 입력해주세요".toString(),Toast.LENGTH_LONG).show();
                }else if(finduser_findpw.getText().toString().trim().equals("")){
                    Toast.makeText(mContext, "비밀번호를 입력해주세요".toString(),Toast.LENGTH_LONG).show();
                }else if(finduser_findpw.getText().toString().trim().length()>20){
                    Toast.makeText(mContext, "비밀번호는 20자 초과 사용 불가합니다.".toString(),Toast.LENGTH_LONG).show();
                }else if(!email_validator.isValidIdPw(finduser_findpw.getText().toString().trim())){
                    Toast.makeText(mContext, "비밀번호는 문자,숫자,특수문자만 사용가능".toString(),Toast.LENGTH_LONG).show();
                }else if(finduser_findpwre.getText().toString().trim().equals("")){
                    Toast.makeText(mContext, "비밀번호확인을 입력해주세요".toString(),Toast.LENGTH_LONG).show();
                }else if(!finduser_findpwre.getText().toString().trim().equals(finduser_findpw.getText().toString().trim())){
                    Toast.makeText(mContext, "비밀번호와 비밀번호확인이 다릅니다.".toString(),Toast.LENGTH_LONG).show();
                }else if(finduser_4digit.getText().toString().trim().equals("")){
                    Toast.makeText(mContext, "메일로 발송된 인증번호를 입력해주세요".toString(),Toast.LENGTH_LONG).show();
                }else if(finduser_4digit.getText().toString().trim().length()!=4){
                    Toast.makeText(mContext, "초기화 PW는 네자리 숫자입니다".toString(),Toast.LENGTH_LONG).show();
                }else if(!finduser_4digit.getText().toString().trim().equals(AuthDigit)){
                    Toast.makeText(mContext, "초기화 PW를 다시 확인해주세요".toString(),Toast.LENGTH_LONG).show();
                }else{
                    ContentValues addRowValue = new ContentValues();

                    addRowValue.put("memId", finduser_findid.getText().toString().trim());
        ...
        finduser_cancel1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(mContext, LoginActivity.class);
                startActivity(intent);
        ...
        finduser_cancel2.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(mContext, LoginActivity.class);
                startActivity(intent);
...
/*
        ...
        finduser_email.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
            ...
            if(s != null){
                ...
                if(jsonParsing(s).trim().equals("2")){
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(mContext, "변경성공".toString(),Toast.LENGTH_LONG).show();
                            Intent intent = new Intent(mContext, LoginActivity.class);
                            startActivity(intent);
                            ((FindUserActivity) mContext).finish();
                        }
        ...
        protected Integer doInBackground(Integer ... values) {
            ...
            if (isCancelled() == false) {
                ...
                try {
                    ...
                    mailSender = new MailSender("hayley2300", "qdgbzdmdmdlgqnmp");
                    mailSender.sendMail("[Basin] 가입인증 " , "인증번호 : "+AuthDigit+" 입니다.", finduser_email.getText().toString().trim());
